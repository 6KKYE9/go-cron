...
func TestWeekday7Alias(t *testing.T) {
	// 周字段 7 作为周日别名，应与 0 等价
	s7, err := Parse("0 0 * * 7")
	if err != nil {
	...
	if !s7.Week[0] || s7.Week[7] {
		t.Fatalf("周=7 应归一到 0: %v", s7.Week)
	}
	if !s7.match(sun) != !s0.match(sun) {
		t.Fatal("周=7 与周=0 命中结果应一致")
	}
}
